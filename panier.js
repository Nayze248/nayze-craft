document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- 2. CHARGEMENT DU PANIER (LOCALSTORAGE) ---
    ...
    window.removeFromCart = function(index) {
        ...
        saveCart();
        updateCartUI();
        renderCartPage(); // Redessine la page panier
    };
    ...
    // Rediriger vers la page panier (Utilisé si un bouton utilise toggleCartDrawer)
    ...
    // Mettre à jour le badge (compteur) de la Navbar
    ...
    // --- 4. AFFICHAGE DYNAMIQUE SUR PANIER.HTML ---
    function renderCartPage() {
        const container = document.getElementById('cartPageContainer');
        if (!container) return; // Si on n'est pas sur panier.html, on arrête la fonction
                ...
                itemsHTML += `
                    <div class="bg-white p-5 rounded-xl shadow-sm border border-gray-200 flex justify-between items-center">
                        <div>
                            ...
                            <p class="text-green-600 font-bold text-md mt-1">${parseFloat(item.price).toFixed(2)} €</p>
                        </div>
            ...
            const summaryHTML = `
                <div class="bg-white p-6 rounded-xl shadow-md border border-gray-200 h-fit space-y-6">
                    ...
                    <div class="flex justify-between text-gray-600">
                        <span>Articles (${window.cart.length})</span>
